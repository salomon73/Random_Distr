...
%                                               %
% Tests randomly generated val. expected        %
% to follow  a Gamma distr. of parameters       %
% (theta, kappa): see function from Gamma.m     % 
%                                               %
% S.Guinchard (12/08/2022)                      %
%-----------------------------------------------%
...
%% generates random number follong this gamma distribution %%
    ...
    disp( strcat('Mean = ', num2str(theta*kappa)) )
    disp(strcat('k = ', num2str(kappa)))
    disp(strcat('theta = ', num2str(theta)))
    for ii=1:nit
        rand_num(ii) = Gamma(Emax, theta, kappa);
    end
    mean(rand_num)
